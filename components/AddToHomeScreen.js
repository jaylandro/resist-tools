...
export default function AddToHomeScreen(props) {
  const DEFAULT_PROMPT = {
    title: "Install Resist Tools on your homescreen?",
    cancelMsg: "Not now",
    installMsg: "Install",
    ...
    for (let key in DEFAULT_CONFIGURATION.customPromptPlatformDependencies) {
      if (
        ...
        if (props.customPromptPlatformDependencies) {
          ...
          options.customPromptPlatformDependencies[key] =
            DEFAULT_CONFIGURATION.customPromptPlatformDependencies[key];
        }
      }
    }
  ...
  function canPrompt() {
    ...
    if (!("serviceWorker" in navigator)) {
      doLog(
        "not displaying call-out because service workers are not supported"
      );
      return false;
    }
